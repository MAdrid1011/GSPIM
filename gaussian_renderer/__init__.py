...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, skip_depth_sort = False):
    """
    Render the scene. 
    ...
    # GSPIM扩展：从传入的参数中获取initial_T（如果有）
    initial_T_param = getattr(pc, '_gspim_initial_T', None)
    ...
    raster_settings = GaussianRasterizationSettings(
        ...
        prefiltered=False,
        debug=pipe.debug,
        initial_T=initial_T_param,  # GSPIM: 初始透射率
        skip_depth_sort=skip_depth_sort  # GSPIM: 跳过深度排序
    )
