...
parent_dir = os.path.join(_repo_root, "submodules", "diff-gaussian-rasterization")
if not os.path.isdir(parent_dir):
    parent_dir = os.path.join(_repo_root, "diff-gaussian-rasterization")
_C = load(
...
class _RasterizeGaussians(torch.autograd.Function):
    ...
    def forward(
        ...
        args = (
            ...
            initial_T if initial_T is not None else torch.empty(0),  # GSPIM: initial_T
            raster_settings.skip_depth_sort,  # GSPIM: skip_depth_sort
        )
...
class GaussianRasterizationSettings(NamedTuple):
    ...
    prefiltered : bool
    debug : bool
    initial_T : torch.Tensor = None  # GSPIM扩展：初始透射率（用于批次增量渲染）
    skip_depth_sort : bool = False  # GSPIM扩展：跳过深度排序（当输入已按深度排序时）
...
class GaussianRasterizer(nn.Module):
    ...
    def forward(self, means3D, means2D, opacities, shs = None, colors_precomp = None, flow_2d = None, ts=None,
                ...
                cov3D_precomp = None, initial_T = None):
        ...
        # 处理initial_T（GSPIM扩展）
        if initial_T is None and raster_settings.initial_T is not None:
            initial_T = raster_settings.initial_T
